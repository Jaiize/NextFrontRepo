...
const NavBar = () => {
  ...
  // NavBar height shrink Mechanism on scroll
  const handleHeightShrink = () => {
    if(typeof window !== 'undefined' && window.pageYOffset > 0) setShrink(true)
    if (typeof window !== 'undefined' && window.pageYOffset === 0) setShrink(false)
  }

  useEffect(() => {
    const controller = new AbortController();
    if(typeof window !== 'undefined'){
      window.addEventListener('scroll', handleHeightShrink, {signal: controller.signal})
    }
